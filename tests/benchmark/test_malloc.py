...
from .test_calc_pipeline import EXPRESSIONS as NATIVE_EXPRESSIONS
...
def test_expression_render_memory(qapp, name: str):
    _render_once(qapp, EXPRESSIONS[name])
...
def test_app_render_memory(qapp, name: str):
    ...
    display.editor.set_plain_text(EXPRESSIONS[name])
    qapp.processEvents()


@pytest.mark.benchmark
@pytest.mark.parametrize("name", list(NATIVE_EXPRESSIONS))
def test_native_tokenize_memory(name: str):
    """Measure C++ tokenizer allocations (std::vector<Token>, std::string)."""
    calc_native.tokenize_string(NATIVE_EXPRESSIONS[name])
...
def test_native_shunting_yard_memory(name: str):
    ...
    tokens = tokenize_string(NATIVE_EXPRESSIONS[name])
