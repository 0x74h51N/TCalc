...
def test_rpn_binary_order( op_ids, token_factory, dummy_calc):
...
def test_rpn_unary_and_postfix( op_ids, token_factory, dummy_calc):
    num, op = token_factory
...
def test_rpn_malformed_raises( op_ids, token_factory, dummy_calc, rpn):
    num, op = token_factory
    it = iter(rpn)
    tokens = []
    for kind in it:
